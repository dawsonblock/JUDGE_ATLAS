...
def resolve_judge_main_root(extract_dir: Path) -> Path:
    ...
    candidates = candidate_judge_main_roots(extract_dir)
    if not candidates:
        raise FileNotFoundError(
            f"could not locate JUDGE-main under extracted archive root: {extract_dir}"
        )
    if len(candidates) > 1:
        raise RuntimeError(
            "multiple JUDGE-main candidates found: "
